...
def parse_gb_t_citation(cf):
    ...
    result = {}
    # Convert full-width characters (letters, digits, punctuation) to half-width
    cf_norm = unicodedata.normalize('NFKC', cf)
    # Ensure [J] / [M] are plain ASCII
...
def is_valid_journal_name(line, title=''):
    ...
    for b in bad:
        if b in line:
            return False
    # Must contain strong journal keyword
    if not re.search(r'(学报|杂志|期刊|大学|学院|研究)', line):
        return False
    # Reject lines that look like book references
    if re.search(r'(《|》|出版社|出版|，第?\d+页)', line):
        return False
    if re.match(r'^[\d\s\.\-NoVol.,（）()\[\]]+$', line):
        return False
    # Reject if identical to title
...
def extract_pdf_meta(pdf_path, basename):
    ...
    try:
        ...
        if re.search(r'独创性声明|毕业(论文|设计)|学位论文\s*$', full_text):
            entry['doc_type'] = 'thesis'
        # Only mark as thesis if filename doesn't suggest it's a journal article with '硕士学位论文' in citation etc.
        if entry.get('doc_type') == 'thesis' and '学位论文' in title:
            pass
        elif entry.get('doc_type') == 'thesis' and not re.search(r'(学位论文|独创性声明)', full_text[:800]):
            # Revert if not strong evidence
            entry['doc_type'] = doc_type
        
        # Book: conservative (filename or epub only)
        ...
        # 3. Year / Volume / Issue / Pages from text if still missing
        ...
        if not entry.get('start_page'):
            ...
            if m:
                ...
                # Reject ISSN-like ranges (4-digit - 4-digit with huge gap)
                if len(s1) == 4 and len(s2) == 4 and (int(s2) - int(s1) > 500 or int(s1) > int(s2)):
                    pass
                else:
                    entry['start_page'] = s1
                    entry['end_page'] = s2
...
json_path = os.path.join(ref_dir, '_extracted_metadata_v6.json')
