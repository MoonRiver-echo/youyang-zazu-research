...
# parts[0] is preamble, then vol, text, vol, text...
...
for vol_title, vol_text in volumes:
    ...
    for p in paras:
        ...
        if any(k in t for k in ['龙','凤','麒麟','蛟','螭','龟','蛇']):
            tags.append('神兽')
        if not tags:
            continue  # Skip non-supernatural entries for this focused site
...
print(f"Total supernatural entries: {len(all_entries)}")
